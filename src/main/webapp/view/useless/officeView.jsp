...
<body
	class="bg-gray-200 font-primary text-primary caret-gray-700 selection:bg-gray-500 selection:text-white h-screen scrollbar-thin scrollbar-thumb-gray-900 scrollbar-track-gray-300 hover:scrollbar-thumb-gray-700">
	<header
		class="py-8 px-36 flex justify-between bg-gray-100 sticky top-0 z-10">
		<a href="/admin/addOfficeItem"
			...
			Add More Products</a>
		<h1 class="text-3xl text-center font-bold">Office Products</h1>
		<a href="/admin"
	...
	<main class="px-36 flex flex-col my-5 gap-5">
		<c:forEach var="temp" items="${products}">
			<c:url var="updateLink" value="/Officeproducts/update">
				<c:param name="productId" value="${temp.id}" />
			</c:url>

			<!-- construct an "delete" link with customer id -->
			<c:url var="deleteLink" value="/Officeproducts/delete">
				<c:param name="productId" value="${temp.id}" />
			</c:url>
			<div
				...
				<div class="flex gap-5 justify-between items-center w-4/5">
					...
					<p class="text-sm truncate ">${temp.description}</p>
					<p>Rs.${temp.price}</p>
					...
					<a href="${updateLink}"><button
						...
						onclick="if (!(confirm('Are you sure you want to delete this customer?'))) return false"><button
